    ...
    <section id='home' className={`flex lg:flex-row flex-col ${styles.paddingY}`} >
      ...
      <div className={`xl:flex-[1.25] flex-1 ${styles.flexStart} flex-col xl:px-0 sm:pl-16 sm:pr-0 px-6 `}>
        ...
        <InView triggerOnce={true}>
          {({ref, inView}) => (
            <h1 ref={ref} className={`${inView ? 'hero-visible delay-1' : 'notVisible'} font-poppins font-bold ss:text-[72px] xs:text-[55px] text-[42px]`}>
        ...
        <InView triggerOnce={true}>
          {({ref, inView}) => (
            <Button ref={ref} styles={`${inView ? 'hero-visible delay-2' : 'opacity-0 translate-y-[50px]'} ss:mt-[60px] mt-10 button-gradient`}content='Get Started' />
          )}
        </InView>

        <InView triggerOnce={true}>
        {({ref, inView}) => (       
          <div ref={ref} className={`${inView ? 'hero-visible delay-3' : 'opacity-0 translate-y-[50px]'} ss:mt-[87px] mt-10 flex flex-row ss:gap-[64px] gap-[50px] items-center justify-start`} >
        ...
        <InView triggerOnce={true}>
        {({ref, inView}) => (  
          <div ref={ref} className={`${inView ? 'hero-visible delay-4' : 'opacity-0 translate-y-[50px]'} flex flex-row mt-[35px]`}>
      ...
      <InView triggerOnce={true}>
        {({ref, inView}) => ( 
          <div ref={ref} className={`${inView ? 'hero-visible delay-5' : 'opacity-0 translate-y-[50px]'} flex-1 flex ${styles.flexCenter} md:my-0 my-10 relative`}>
